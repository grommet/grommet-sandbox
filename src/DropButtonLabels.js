...
export default class LabelMultiSelect extends Component {
  ...
  select = (label) => {
    ...
    this.setState({
      animation: {
        ...
        [newSelected.length - 1]: { type: 'fadeIn', duration: 400 },
      },
      open: undefined,
  ...
  remove = (index) => {
    this.setState({
      animation: {
        ...this.state.animation,
        [index]: { type: 'fadeOut', duration: 200 },
      },
      open: undefined,
    }, () => {
      const newSelected = [...this.state.selected];
      ...
      setTimeout(() => {
        this.setState({
          animation: { ...this.state.animation, [index]: undefined },
          available: labels.filter(value => newSelected.indexOf(value) === -1),
          selected: newSelected,
        });
      }, 200);
  ...
  reset = (event) => {
    ...
    this.setState({
      animation: {
        ...this.state.animation,
        all: { type: 'fadeOut', duration: 200 },
      },
      available: labels,
    }, () => {
      setTimeout(() => {
        this.setState({
          animation: { ...this.state.animation, all: undefined },
          selected: [],
        });
      }, 200);
    });
  ...
  render() {
    const {
      animation, available, open, selected,
    } = this.state;
